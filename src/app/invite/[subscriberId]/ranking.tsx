                        ...
                        return(
                            <div key={item.id} className='relative rounded-xl bg-gray-700 border border-gray-600 p-6 flex flex-col justify-center gap-3'>
                                <span className='text-sm text-gray-300 leading-none'>
                                    <span className='font-semibold'>{rankinPosition}°</span> | {item.name}
                                </span>
                                <span className='font-heading text-2xl font-semibold text-gray-200 leading-none'>{item.score}</span>

                                { rankinPosition === 1 && <Image src={gold} alt='' className='absolute top-0 right-8'/> }
                                { rankinPosition === 2 && <Image src={silver} alt='' className='absolute top-0 right-8'/> }
                                { rankinPosition === 3 && <Image src={cooper} alt='' className='absolute top-0 right-8'/> }
                            </div>
                        )
                    })}
                </div>
